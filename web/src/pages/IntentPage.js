...
// deployment stuff
const intentEndpoint = (process.env.NODE_ENV === 'development')
  ? 'http://localhost:3000/interest'
  : 'https://api.virgilcard.com/interest'
...
export default function IntentPage() {
  ...
  const [invalidInputCompany, setInvalidInputCompany] = useState(false)
  const [invalidInputEmail, setInvalidInputEmail] = useState(false)
  const [invalidInputNotes, setInvalidInputNotes] = useState(false)

  const handleSubmit = () => {
    if (inputName === '') setInvalidInputName(true)
    if (inputCompany === '') setInvalidInputCompany(true)
    if (inputEmail === '') setInvalidInputEmail(true)
    if (inputNotes === '') setInvalidInputNotes(true)
    ...
    if (inputName === ''
      ...
      setSubmitted(true)
      axios.post(intentEndpoint, {name: inputName, company: inputCompany, email: inputEmail, notes: inputNotes})
      .then(response => {
      ...
      <Div web={12} style={{minHeight:mobileView?'80vh':'70vh', backgroundColor:'white', 
        ...
        <Div web={10} mobile={10} style={{flexDirection:mobileView?'column':'row'}}>
          ...
          ? <Div web={6} mobile={12} style={{flexDirection:'column',
              alignItems:mobileView?'center':'flex-end', justifyContent:'flex-end'}}>
              <Div web={8} mobile={12} style={{flexDirection:'row', alignItems:'center', padding:'10px 0px 10px 0px'}}>
                <InputTemplate 
                  input={inputName}
                  setInput={setInputName}
                  placeholder={'Your Name'}
                  invalidInput={invalidInputName}
                  setInvalidInput={setInvalidInputName}
                  />
              </Div>
              ...
              <Div web={8} mobile={12} style={{flexDirection:'row', alignItems:'center', padding:'10px 0px 10px 0px'}}>
                <InputTemplate 
                  input={inputCompany}
                  setInput={setInputCompany}
                  placeholder={'Your Company'}
                  invalidInput={invalidInputCompany}
                  setInvalidInput={setInvalidInputCompany}
                  />
              </Div>
              ...
              <Div web={8} mobile={12} style={{flexDirection:'row', alignItems:'center', padding:'10px 0px 10px 0px'}}>
                <InputTemplate 
                  ...
                  invalidInput={invalidInputEmail}
                  setInvalidInput={setInvalidInputEmail}
                  />
              </Div>
              ...
              <Div web={8} mobile={12} style={{flexDirection:'row', alignItems:'center', padding:'10px 0px 10px 0px'}}>
                <TextAreaTemplate 
                  ...
                  placeholder={'Any additional terms, considerations, caveats, or requirements'}
                  invalidInput={invalidInputNotes}
                  setInvalidInput={setInvalidInputNotes}
                  />
              </Div>

              <Div web={8} mobile={12} style={{flexDirection:'row', alignItems:'center', padding:'10px 0px 10px 0px'}}>
                <StyledButton style={{margin:'30px 0px 0px 0px'}} onClick={handleSubmit}>
                  Submit
                </StyledButton>
              </Div>
            </Div>
